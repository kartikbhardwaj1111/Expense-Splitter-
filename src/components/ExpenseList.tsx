...
function ExpenseList({ expenses, onRemoveExpense }: ExpenseListProps) {
              ...
              <div className="p-4 flex justify-between items-center cursor-pointer transition-colors hover:bg-gray-100">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="bg-transparent text-red-500 px-2 py-1 transition-colors hover:bg-red-100 rounded ml-2"
                    aria-label="Delete"
                  >
                    🗑️
                  </button>
              ...
              {expandedExpense === expense.id && (
                <div className="px-4 py-3 bg-white border-t border-gray-200">
                  <div className="text-sm text-gray-600 space-y-1">
                    <p><strong>Split Type:</strong> {expense.splitType === 'equal' ? 'Equal Split' : 'Custom Amounts'}</p>
                    <p><strong>Split Between:</strong> {expense.splitBetween.join(', ')}</p>
                    {expense.splitType === 'equal' && (
                      <p><strong>Amount per person:</strong> ${(expense.amount / expense.splitBetween.length).toFixed(2)}</p>
                    )}
                    {expense.splitType === 'custom' && expense.customAmounts && (
                      <div>
                        <p><strong>Custom amounts:</strong></p>
                        <ul className="ml-4">
                          {Object.entries(expense.customAmounts).map(([person, amount]) => (
